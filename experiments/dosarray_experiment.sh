...
function dosarray_http_experiment() {
  ...
  do
    if [ "${TOTAL_RUNS}" -eq "1" ]
    then
      export DOSARRAY_DESTINATION_DIR="${PRE_DESTINATION_DIR}/"
    else
      echo "Starting run ${RUN} of ${TOTAL_RUNS}"
      export DOSARRAY_DESTINATION_DIR="${PRE_DESTINATION_DIR}/${RUN}/"
    fi
    ...
    echo "  Writing to ${DOSARRAY_DESTINATION_DIR}"
    ...
    then
      ...
      echo "#  Generated by DoSarray v${DOSARRAY_VERSION} on $(date)" > "${DOSARRAY_DESTINATION_DIR}/dosarray.stdout"
      echo "stdout collection is disabled (by default)" >> "${DOSARRAY_DESTINATION_DIR}/dosarray.stdout"
      echo "To enable set DOSARRAY_INCLUDE_STDOUTERR=1. See USAGE.md for more info." >> "${DOSARRAY_DESTINATION_DIR}/dosarray.stdout"

      echo "#  Generated by DoSarray v${DOSARRAY_VERSION} on $(date)" > "${DOSARRAY_DESTINATION_DIR}/dosarray.stderr"
      echo "stderr collection is disabled (by default)" >> "${DOSARRAY_DESTINATION_DIR}/dosarray.stderr"
      echo "To enable set DOSARRAY_INCLUDE_STDOUTERR=1. See USAGE.md for more info." >> "${DOSARRAY_DESTINATION_DIR}/dosarray.stderr"
    else
      echo "#  Generated by DoSarray v${DOSARRAY_VERSION} on $(date)" > "${DOSARRAY_DESTINATION_DIR}/dosarray.stdout"
      cat ${STD_OUT} >> ${DOSARRAY_DESTINATION_DIR}/dosarray.stdout
      echo "#  Generated by DoSarray v${DOSARRAY_VERSION} on $(date)" > "${DOSARRAY_DESTINATION_DIR}/dosarray.stderr"
      cat ${STD_ERR} >> ${DOSARRAY_DESTINATION_DIR}/dosarray.stderr

      rm ${STD_OUT}
      rm ${STD_ERR}
    fi

    mv ${MANIFEST} ${DOSARRAY_DESTINATION_DIR}/dosarray.manifest
