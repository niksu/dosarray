...
cd ${DESTINATION_DIR}
command time ${DOSARRAY_SCRIPT_DIR}/src/generate_availability_chart.py "${DOSARRAY_LOG_NAME_PREFIX}*.log" > availability.data

#generate availability data for 'availability over time' plot
DOSARRAY_NHIST_RESULT=1 python ${DOSARRAY_SCRIPT_DIR}/src/generate_availability_chart.py "${DOSARRAY_LOG_NAME_PREFIX}*.log" > availability_filtered.data
